...
calculate_accuracy <- function(proba, y_true, top_n = 3, low_n = 3) {
  ...
  # Low-N Accuracy
  low_n_indices <- sorted_indices[(ncol(proba) - low_n + 1):ncol(proba), , drop = FALSE]  # Indices of low-N classes
...
example_input <- data.frame(
  ...
  AREA.NAME = "CENTRAL",   # Categorical
  period = "late_night"       # Categorical
)
